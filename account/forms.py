...
class CheckUsernameForm(forms.Form):
    username = forms.CharField(max_length=50, widget=forms.TextInput(attrs={'class': 'peer w-full rounded-lg border-none bg-transparent p-4 text-left placeholder-transparent focus:outline-none focus:ring-0', "placeholder": "نام کاربری"}))
    ...
    def clean_username(self):
        ...
        if re.fullmatch(email, username):
            return username
        elif re.fullmatch(phone, username):
            return username
        else:
            raise forms.ValidationError('ورودی معتبر نیست. لطفا یک ایمیل معتبر یا شماره تلفن ۱۱ رقمی وارد کنید.')


class LoginForm(forms.Form):
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'peer w-full rounded-lg border-none bg-transparent p-4 text-left placeholder-transparent focus:outline-none focus:ring-0', 'placeholder': "کلمه عبور"}))
